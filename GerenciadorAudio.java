...
public class GerenciadorAudio {
    ...
    private static void carregarClipVozRaquel() {
        ...
        try {
            // Tenta copiar do diretório de Downloads do usuário caso esteja lá mas não na pasta do jogo
            java.io.File destFile = new java.io.File(JogoAudrey.resolvePath("WhatsApp-Audio-2026-08-01-at-10.24.00-PM.wav"));
            if (!destFile.exists()) {
                String userHome = System.getProperty("user.home");
                java.io.File srcFile = new java.io.File(userHome + "/Downloads/WhatsApp-Audio-2026-08-01-at-10.24.00-PM.wav");
                if (srcFile.exists()) {
                    try {
                        ...
                        System.out.println("[VOZ] Audio da Raquel copiado automaticamente dos Downloads para o jogo!");
                    } catch (Exception ex) {
                        System.err.println("[VOZ] Erro ao copiar audio da Raquel: " + ex.getMessage());
                    }
                }
            }
            ...
            // 1. Busca dinâmica por arquivos de áudio do WhatsApp ou Raquel na pasta do jogo
            java.io.File pasta = new java.io.File(JogoAudrey.resolvePath("."));
            if (pasta.isDirectory()) {
                java.io.File[] arquivos = pasta.listFiles();
                if (arquivos != null) {
                    for (java.io.File arq : arquivos) {
                        String nome = arq.getName().toLowerCase();
                        if (nome.contains("whatsapp") || nome.contains("raquel")) {
                            f = arq;
                            break;
                        }
                    }
            ...
            if (f == null || !f.exists()) {
                String[] arquivosPossiveis = {
                    "WhatsApp-Audio-2026-08-01-at-10.24.00-PM.wav",
                    "WhatsApp-Audio-2026-08-01-at-10.24.00-PM.mp3",
                    "WhatsApp-Audio-2026-08-01-at-10.24.00-PM.ogg",
                    "WhatsApp-Audio-2026-08-01-at-10.24.00-PM",
                    "WhatsApp Audio 2026-08-01 at 10.24.00 PM.wav",
                    "WhatsApp Audio 2026-08-01 at 10.24.00 PM.mp3",
                    "WhatsApp Audio 2026-08-01 at 10.24.00 PM.ogg",
                    "WhatsApp Audio 2026-08-01 at 10.24.00 PM",
            ...
            if (f == null || !f.exists()) {
                logVoz("Aguardando arquivo de voz da Raquel: 'WhatsApp Audio 2026-08-01 at 10.24.00 PM.wav' ou 'vozdopersonagemraquel.wav'");
                return;
            }
